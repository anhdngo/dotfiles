...
│
│  # WSL-only
│  (winhome link is created by the WSL run_once script below)
│
...
│   (Windows Terminal settings are patched in place by setup.ps1)
│
...
### 6. WSL (`run_once_after_60-wsl.sh.tmpl`)
...
- `~/winhome` → the Windows user profile. Resolve the target at apply time:
  ...
  to globbing `/mnt/c/Users/`. Done in a run_once script (rather than a `symlink_`
  template) so cmd.exe isn't invoked on every template render; the script is a no-op
  on non-WSL machines.
...
4. **Quake terminal on Ctrl+\\**: patch Windows Terminal's `LocalState\settings.json`
   in place (backing up the existing file), adding a quake-mode keybinding on `ctrl+\`
   while preserving existing settings. This is done in Windows Terminal
